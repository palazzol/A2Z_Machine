...
int analyze_exefile( void )
{
   ...
   while ( ( c = gfp.read() ) > -1 )
   {
      ...
      else if ( ++i == MAGIC_END )
      {
         ...
         story_offset = gfp.read();
         story_offset += 256L * gfp.read();
         story_offset += 65536L * gfp.read();
         return TRUE;
      }
   }
...
void set_names( const char *storyname )
{
   ...
   // remove path from base filename
   per_pos = strrchr( storyname, '/' );
   storyname = ( per_pos ) + 1;
...
static int save_restore( const char *file_name, int flag )
{
   ...
   else
   {
      ...
      if ( flag == GAME_SAVE )
      {
         ...
         if ( status == 0 && tfp.write( stack, sizeof ( stack ) ) != 1 )
            status = 1;
         if ( little_endian )
            swap_bytes( stack, sizeof ( stack ) );
      }
      else if ( flag == GAME_RESTORE )
      {
         if ( little_endian )
            swap_bytes( stack, sizeof ( stack ) );
         if ( status == 0 && tfp.read( stack, sizeof ( stack )) != 1 )
            status = 1;
         if ( little_endian )
            swap_bytes( stack, sizeof ( stack ) );
      }
      ...
      if ( flag == GAME_SAVE )
      {
         if ( status == 0 && tfp.write( datap, h_restart_size ) != 1 )
            status = 1;
      }
      else if ( flag == GAME_RESTORE )
      {
         if ( status == 0 && tfp.read( datap, h_restart_size ) != 1 )
            status = 1;
      }
      else
#endif /* !defined(USE_QUETZAL) */
...
void script_char( int c )
{
   ...
   if ( scripting == ON && scripting_disable == OFF && ( c == '\r' || ( isprint( c ) ) ) )
   {
      //putc( c, sfp );
      sfp.write((uint8_t *)&c,1);
   }
...
void script_new_line( void )
{
   ...
   //script_char( '\n' );
   script_char( '\r' ); // Arduino uses this
